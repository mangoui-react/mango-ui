...
function Root({ className, children, ...props }: RootProps) {
  return (
    <table
      {...props}
      className={cn('overflow-hidden text-left w-full border border-slate-700', className)}
    >
      {children}
    </table>
  );
...
function Header({ className, children, ...props }: HeaderProps) {
  return (
    <thead {...props} className={cn('bg-slate-700', className)}>
      {children}
    </thead>
  );
}
...
function Body({ className, children, ...props }: BodyProps) {
  return (
    <tbody {...props} className={cn('text-slate-100', className)}>
      {children}
    </tbody>
  );
}
...
function NameCell({ className, children, ...props }: NameCellProps) {
  return (
    <th
      {...props}
      className={cn('p-3 text-sm text-green-200 shadow-[inset_0_-1px_rgb(51,65,85)]', className)}
    >
      {children}
    </th>
  );
...
function Cell({ className, children, ...props }: CellProps) {
  return (
    <td {...props} className={cn('p-3 text-sm shadow-[inset_0_-1px_rgb(51,65,85)]', className)}>
      {children}
    </td>
  );
}
